@extends('account-backend::layouts.bootstrap_modal')
...
vertical-align: middle!important;">
                                    ...
                                    @foreach($theme->theme->items as $k=> $item)
                                        ...
                                        @if($item->is_default)
                                            <script>
                                                ...
                                                CommitMiniCode_data.ext_json.ext.appid="{{request('uuid')}}";
                                                CommitMiniCode_data.ext_json.ext.wechatappid="{{request('wechatappid')}}";
                                                $('#theme-no').show();
                    ...
                    <tr>
                        <td></td>
                        <td class="col-sm-2">
                            <img class="pull-left" id="img_theme" src="" width="25%" height="25%" alt="">
                        </td>
                    </tr>
...
vertical-align: middle!important;">
            ...
            <button style="position: absolute;bottom:-50px;right:20px" data-dismiss="modal"
                    class="btn btn-primary pull-right" onclick="Submission()">生成体验版
            </button>
...
@section('footer')
    ...
    <button type="button" class="btn btn-link" data-dismiss="modal" style="margin-right:120px;">取消</button>
    ...
    function Submission() {
        ...
        $.post(CommitMiniCode_url, CommitMiniCode_data, function (result) {
            ...
            if (result.status) {
                ...
                $('#experience_code').show()
                $('#CommitMiniCodeUpload').show()
            } else {
                swal({
                    title: "上传小程序代码失败",
                    text: result.message,
